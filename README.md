...
公開勤務表に表示するメモと変更理由も共有します。DBの内部情報や編集履歴は返しません。
...
| `/api/v1/schedule?from=2026-07-28&days=14` | 指定日から指定日数の勤務表 |
| `/api/v1/month?year=2026&month=8` | 指定月の勤務表 |
| `/api/v1/holidays?year=2026` | 指定年の日本の祝日と出典・暫定状態 |
| `/shift/calendar.ics` | デイリーステーション等へ同期する勤務カレンダー |

公開APIは `GET` と `HEAD` に対応し、ブラウザ等から利用できるよう
`Access-Control-Allow-Origin: *` を返します。仕様変更が必要になった場合は
`/api/v2/` を追加し、`v1` の応答形式は維持します。

勤務日の共通形式では、`status` は実際の仕事・休み、`type` は通常勤務・
有給・通常休み・休出の表示種別です。`cycleGroup` はミニウィジェットで
次の仕事・休みを探すための勤務周期上の区分で、有給は `work`、休出は
`rest` として扱います。

勤務カレンダーは、取得日の31日前から365日後までを終日予定として配信します。
各日の予定IDは固定なので、再同期しても重複しません。SwitchBotアプリでは
デイリーステーションの「カレンダー設定」→「同期」に
`https://mirachan.net/shift/calendar.ics` を登録します。
